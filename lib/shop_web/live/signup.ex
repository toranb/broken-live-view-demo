defmodule ShopWeb.Live.Signup do
            ...
            <fieldset class="flex flex-col md:w-full" <%= is_disabled(@changeset) %>>
              ...
              <div class="flex flex-col md:w-full">
                ...
                <div aria-live="polite" aria-hidden="<%= aria_hidden(f, @changeset, :password) %>" id="form_password_detail" class="<%= detail_error(f, @changeset, :password) %> detail bg-shop-info pt-2 pb-2 pl-5 pr-4 rounded rounded-t-none relative block border-t-0 border">
                  <span aria-hidden="<%= aria_hidden(f, @changeset, :password) %>" class="text-left text-sm text-shop-black">Password must contain a letter and a number, and be minimum of 8 characters</span>
                </div>
              </div>
  ...
  def mount(_params, %{"membership_id" => membership_id}, socket) do
    changeset =
      Shop.Form.changeset(%Shop.Form{}, %{membership_id: membership_id})
